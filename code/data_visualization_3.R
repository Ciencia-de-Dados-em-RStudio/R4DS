...
#Source: https://jrnold.github.io/r4ds-exercise-solutions/data-visualisation.html#fnref2
...
?mpg 
#The drv variable is a categorical variable which categorizes cars into f=front-wheels, r=rear-wheels, or 4=four-wheel drive.
#See: https://en.wikipedia.org/wiki/Car_layout
...
#Since categorical variables typically take a small number of values, there are a limited number of unique 
#combinations of (x, y) values that can be displayed. 
#In this data, drv takes 3 values and class takes 7 values, meaning that there are only 21 values that could be 
#plotted on a scatterplot of drv vs. class. 
#In this data, there 12 values of (drv, class) are observed.

#Obs:count() => Count observations by group.
# Belongs to dplyr package
...
#and when all (x, y) values are unique.

##Warning: The following code uses functions introduced in a later section. 
#Come back to this after reading section 7.5.2, which introduces methods for plotting two categorical variables. 

#The first is geom_count() which is similar to a scatterplot but uses the size of the points to show the number 
#of observations at an (x, y) point.
ggplot(mpg, aes(x = class, y = drv)) +
  geom_count()
...
#Obs: %>% symbol is a "pipe". It belongs to package magrittr and it is inside "tidyverse".
#The code is arranged in a logical order of how one think about a task: data->transform->aggregate, 
#which is also the same order as the code will execute!
...
#The argumentcolour = "blue" is included within the mapping argument, and as such, it is treated as an aesthetic, 
...
#For those lists, I considered any variable that was non-numeric was considered categorical and any 
#variable that was numeric was considered continuous. 
...
#Even though displ has In some sense, due to measurement and computational constraints all numeric variables are discrete (). 
#But unlike the categorical variables, it is possible to add and subtract these numeric variables in a meaningful way. 
#The typology of levels of measurement is one such typology of data types.
#In this case the R data types largely encode the semantics of the variables; e.g. integer variables are stored as integers, 
#categorical variables with no order are stored as character vectors and so on. 
#However, that is not always the case. Instead, the data could have stored the categorical class variable as an integer 
#with values 1–7, where the documentation would note that 1 = “compact”, 2 = “midsize”, and so on. (See: https://r4ds.had.co.nz/factors.html)  
#Even though this integer vector could be added, multiplied, subtracted, and divided, those operations
#would be meaningless.
#Fundamentally, categorizing variables as “discrete”, “continuous”, “ordinal”, “nominal”, “categorical”, etc. 
#is about specifying what operations can be performed on the variables. 
#Discrete variables support counting and calculating the mode. Variables with an ordering support sorting 
#and calculating quantiles. 
#Variables that have an interval scale support addition and subtraction and operations such as taking the mean 
#that rely on these primitives. 
#In this way, the types of data or variables types is an information class system, something that is beyond 
#the scope of R4DS but discussed in Advanced R (See: http://adv-r.had.co.nz/OO-essentials.html#s3).
...
ggplot(data = mpg, 
       aes(x = displ, y = hwy, size = cty)) +
  geom_point()

#When mapped to size, the sizes of the points vary continuously as a function of their size.
ggplot(data = mpg, 
       aes(x = displ, y = hwy, shape = cty)) +
  geom_point()
#> Error: A continuous variable can not be mapped to shape

#When a continuous value is mapped to shape, it gives an error. 
#Though we could split a continuous variable into discrete categories and use a shape aesthetic, 
#this would conceptually not make sense. 
#A numeric variable has an order, but shapes do not. It is clear that smaller points correspond 
#to smaller values, or once the color scale is given, which colors correspond to larger or smaller values. 
#But it is not clear whether a square is greater or less than a circle.

#Exercise 3.3.4
#What happens if you map the same variable to multiple aesthetics?
ggplot(data = mpg,
       aes(x = displ, y = hwy, colour = hwy, size = displ)) + 
  geom_point()

#In the above plot, hwy is mapped to both location on the y-axis and color, and displ is mapped to both 
#location on the x-axis and size.
...
#Disadvantages of encoding the class variable with facets instead of the color aesthetic include the difficulty of comparing the values of observations between categories 
#since the observations for each category are on different plots. 
...
#Since encoding class within color also places all points on the same plot, it visualizes the unconditional relationship between the x and y variables; 
...
#Jittering will still work with color. But jittering will only work well if there are few points and the classes do not overlap much, otherwise, 
#the colors of areas will no longer be distinct, and it will be hard to pick out the patterns of different categories visually. 
...
#The nrow and ncol arguments are unnecessary for facet_grid() since the number of unique values 
#of the variables specified in the function determines the number of rows and columns.
...
#This code produces a scatter plot with display on the x-axis, hwy on the y-axis, and the points colored by drv. 
...
#In that plot, there is no legend. Removing the show.legend argument or setting show.legend = TRUE 
#will result in the plot having a legend displaying the mapping between colors and drv.
ggplot(data = mpg) +
  geom_smooth(mapping = aes(x = displ, y = hwy, colour = drv))
#`geom_smooth()` using method = 'loess' and formula 'y ~ x'

#In the chapter, the legend is suppressed because with three plots, adding a legend to only the last plot 
#would make the sizes of plots different. Different sized plots would make it more difficult to see how 
#arguments change the appearance of the plots. The purpose of those plots is to show the difference between 
#no groups, using a group aesthetic, and using a color aesthetic, which creates implicit groups. 
#In that example, the legend isn’t necessary since looking up the values associated with each color 
...
#See graphs on “documents’ folder!
...
# See "Complementary geoms and stats" file on 'documents' folder!
...
# See list on https://ggplot2.tidyverse.org/reference/ !
...
#The relationship between cty and hwy is clear even without jittering the points but jittering 
#shows the locations where there are more observations.
...
#Whe nwidth = 0 there is no horizontal jitter.
ggplot(data = mpg, mapping = aes(x = cty, y = hwy)) +
  geom_jitter(width = 0)
...
#The default values of height and width are defined to be 80% of the resolution() of the data, which is the smallest non-zero 
#distance between adjacent values of a variable. 
#When x and y are discrete variables, their resolutions are both equal to 1, and height = 0.4 and width = 0.4 since the jitter 
#moves points in both positive and negative directions.

#The default values of height and width in geom_jitter() are non-zero, so unless both height and width are explicitly set
#set 0, there will be some jitter.
ggplot(data = mpg, mapping = aes(x = cty, y = hwy)) +
  geom_jitter()
...
#For example, in the following example, a third variable mapped to color is added to the plot.
#In this case, geom_count() is less readable than geom_jitter() when adding a third variable as a color aesthetic.
...
#Combining geom_count() with jitter, which is specified with the position argument to geom_count() rather than its own geom, 
#helps overplotting a little.
ggplot(data = mpg, mapping = aes(x = cty, y = hwy, color = class)) +
...
#This position adjustment does not change the vertical position of a geom but moves the geom horizontally to avoid overlapping other geoms. See the documentation for position_dodge2() for additional discussion on how it works.
...
#See Cleveland (1993b), Cleveland (1994), Cleveland (1993a), Cleveland, McGill, and McGill (1988), Heer and Agrawala (2006)
#for discussion on how the aspect ratio of a plot affects perception of the values it encodes, evidence that 45-degrees is generally 
#the optimal aspect ratio, and methods to calculate the optimal aspect ratio of a plot. 
